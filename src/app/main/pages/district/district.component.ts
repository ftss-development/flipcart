...
export class DistrictComponent implements OnInit, AfterViewInit {
    ...
    allFilters = ['District Name', 'District Short Name', 'Is Active'];
    color = 'accent';
    errors = [];
    formErrors = {
        districtName: '',
...
// It would be better to have this strongly typed to match all the possible filtering criteria.
    ...
    displayedColumns: CustomColumnSchema[] = [
        ...
        {
            position: 1,
            dataColumnName: 'districtName',
            headerColumnName: 'District Name',
            isActive: true,
            isMandatory: true,
            isHidden: false
        },
        {
            position: 2,
            dataColumnName: 'districtShortName',
            headerColumnName: 'District Short Name',
            isActive: true,
            isMandatory: true,
            isHidden: false
        },
        ...
        {
            position: 10, dataColumnName: 'edit', headerColumnName: 'Edit', isActive: true, isMandatory: false,
            isHidden: true
        },
        {
            position: 11, dataColumnName: 'delete', headerColumnName: 'Delete', isActive: true, isMandatory: false,
            isHidden: true
        },
    ];
    ...
    constructor(
        ...
        /*this.filterSelectObj = [
            {
                name: 'District Id',
                columnProp: 'districtId',
                options: []
            }, {
                name: 'District Name',
                columnProp: 'districtName',
                options: []
            }, {
                name: 'District Short Name',
                columnProp: 'districtShortName',
                options: []
            }, {
                name: 'City Name',
                columnProp: 'city',
    ...
    */
    ...
    getDistrict(districtId: string) {
        // @ts-ignore
        return this.district$ = this.service.getDistrict(districtId);
    }

    loadGridView() {

        const districts$ = this.service.getDistricts(this.currentPage, this.pageSize)
            .pipe(
                map(project => project),
            );
        // @ts-ignore
        this.districts$ = districts$.pipe(map(project => project.items));

        return districts$.subscribe(project => {
    ...
    save(formDirective?: FormGroupDirective) {
        if (this.formGroup.valid) {
            ...
            this.service.saveDistrict(district)
    ...
    update(formDirective?: FormGroupDirective) {
        if (this.formGroup.valid) {
            ...
            this.service.updateDistrict(changes).subscribe(response => this.snackBarNotifierService.showNotification(':: Successfully updated!!!', 'Dismiss', 'success'), error => {
                //console.log(error);
                const validationErrors = error.error.errors as ValidationErrors;
                this.snackBarNotifierService.showNotification(':: Duplicate entry found!!!', 'Dismiss', 'error');
    ...
    edit(row: { districtId: string, countryId: string, stateId: string }) {
        ...
        this.getDistrict(row.districtId);
        this.rebuildForm();
    }

    delete(row: any) {
        const id = row.districtId;
        const dialogRef = this.openDialog(row.districtName);
        ...
        return this.district$
            .pipe(finalize(() => {
                this.formGroup = this.fb.group({
                    districtName: new FormControl(this.district.districtName, [Validators.required, Validators.minLength(3), Validators.maxLength(45)])
                    ,
                    districtShortName: new FormControl(this.district.districtShortName, [Validators.required, Validators.minLength(2), Validators.maxLength(15)])
                    ,
                    districtId: new FormControl(this.district.districtId)
                    ,
                    countryId: new FormControl(this.district.countryId, [Validators.required])
                    ,
                    stateId: new FormControl(this.district.stateId, [Validators.required])
                    ,
                    cityId: new FormControl(this.district.cityId, [Validators.required])
                    ,
                    isActive: new FormControl(this.district.isActive ?? true)
                    ,
